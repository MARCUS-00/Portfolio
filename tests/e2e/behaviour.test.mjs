...
test("every route responds, and unknown addresses get the 404 page", async () => {
  await withPage({}, async (page) => {
    for (const [route, h1] of [["/", "Manoj Kumar G"], ["/work/", "Work"], ["/experience/", "Experience"], ["/skills/", "Skills"],
      ["/about/", "About"], ["/contact/", "Get in touch"], ["/p/osteoscan/", "OsteoScan"], ["/p/olist/", "Olist analytics platform"],
...
test("keyboard: skip link, visible focus, and the topology follows focus", async () => {
  await withPage({ viewport: { width: 1440, height: 900 } }, async (page) => {
    ...
    assert.match(await page.textContent("#topoNote"), /hundred thousand orders/);
...
test("topology: hover follows a path on wide screens; one lane at a time on phones", async () => {
  ...
  await withPage({ viewport: { width: 390, height: 844 }, hasTouch: true, isMobile: true }, async (page) => {
    await page.goto(u("/"));
    assert.equal(await page.getAttribute("#topo", "data-active"), "osteoscan");
    ...
    await page.waitForFunction(() => document.getElementById("topo").getAttribute("data-active") === "osteoscan");
  });
});
...
test("skills map: pointing at a skill highlights its system and dims the rest", async () => {
  await withPage({ viewport: { width: 1440, height: 900 } }, async (page) => {
    await page.goto(u("/skills/"));
    await page.hover('.skill:has(.s-n:text-is("SQL"))');
    const dimmed = await page.$$eval(".skill.dim .s-n", (els) => els.map((e) => e.textContent));
    assert.ok(dimmed.includes("TensorFlow") && !dimmed.includes("DAX") && dimmed.includes("Power BI") && !dimmed.includes("SQL"));
    assert.ok(await page.$eval('.proof[data-p="olist"]', (a) => a.classList.contains("rel")));
    await page.hover('.skill:has(.s-n:text-is("Python")) .proof[data-p="c3i"]');
    assert.ok((await page.$$eval(".skill.dim .s-n", (els) => els.map((e) => e.textContent))).includes("SQL"), "one proof link narrows to its system");
    await page.mouse.move(5, 5);
    assert.equal(await page.$$eval(".skill.dim", (e) => e.length), 0);
  });
});
...
test("contact and external links are real and safe", async () => {
  await withPage({}, async (page) => {
    ...
    assert.deepEqual(links.map((l) => l.href), ["mailto:mailtomanojkumar07@gmail.com", "tel:+919738027546", "https://github.com/MARCUS-00", "/Manoj_Kumar_Resume.pdf"]);
    for (const l of links.filter((x) => x.href.startsWith("https://"))) assert.deepEqual([l.target, l.rel], ["_blank", "noopener noreferrer"]);
    assert.ok(links[3].download);
...
test("reduced motion: nothing animates and final values show immediately", async () => {
  await withPage({ reducedMotion: "reduce", viewport: { width: 1440, height: 900 } }, async (page) => {
    ...
    assert.equal((await page.textContent(".moment .mv")).trim(), "61.5%");
...
test("with motion: reveals and count-ups end at the real values", async () => {
  await withPage({ viewport: { width: 1440, height: 900 } }, async (page) => {
    ...
    await page.waitForTimeout(700);
    for (const v of await page.$$eval(".moment .mv", (els) => els.map((e) => e.textContent))) assert.ok(["61.5%", "0.2308", "0.54"].includes(v), v);
